...
export function Hero() {
  ...
  return (
    <>
      <section className="relative pt-36 pb-0 overflow-hidden bg-white dark:bg-gray-900">

        <div className="container max-w-7xl mx-auto relative z-10 px-4 sm:px-6">
          <div className="text-center space-y-6">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium bg-[#0C5F4C]/10 dark:bg-[#10B981]/20 text-[#0C5F4C] dark:text-[#10B981] border border-[#0C5F4C]/20 dark:border-[#10B981]/30 animate-fade-in-down">
              {t('earlyAccessBadge')}
            </div>

            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-[80px] font-bold tracking-tight leading-[1.1] text-gray-900 dark:text-gray-100 animate-fade-in-up animate-delay-100 animate-fill-both px-2 break-words hyphens-auto">
              {t('title')}{' '}
              <br className="hidden md:block" />
              <span className="text-[#0C5F4C] dark:text-[#10B981]">{t('titleHighlight')}</span>
            </h1>

            <p className="text-lg sm:text-xl md:text-[22px] lg:text-[24px] text-gray-600 dark:text-gray-300 max-w-3xl mx-auto font-normal leading-relaxed tracking-tight animate-fade-in-up animate-delay-200 animate-fill-both px-2">
              {t('descriptionPart1')}{t('descriptionPart2')}
            </p>

            {/* Email Input & Buttons with Glow Effect */}
            <div className="relative max-w-2xl mx-auto pt-4 group animate-scale-in animate-delay-300 animate-fill-both px-4">
              ...
              <div className="flex flex-col gap-3 justify-center items-stretch">
                {/* Email Input - Standalone on mobile, integrated on desktop */}
                <div className="relative w-full flex items-center rounded-full border-2 border-[#0C5F4C]/40 dark:border-[#10B981]/40 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-sm">
                  <input
                    type="email"
                    placeholder={t('emailPlaceholder')}
                    className="w-full h-14 pl-4 pr-4 md:pr-40 text-[15px] focus:outline-none bg-transparent border-none text-gray-900 dark:text-gray-100 placeholder:text-gray-500 dark:placeholder:text-gray-400"
                  />
                  {/* Button integrated on desktop */}
                  <Button
                    ...
                    className="hidden md:flex absolute right-0 h-14 bg-[#0C5F4C] hover:bg-[#0A4F3E] dark:bg-[#10B981] dark:hover:bg-[#059669] text-white text-[15px] font-medium px-6 rounded-full whitespace-nowrap pointer-events-auto relative overflow-hidden transition-all duration-300 items-center justify-center"
                  >
                    <span
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <Button
                    onClick={() => setShowAuth(true)}
                    className="md:hidden w-full bg-[#0C5F4C] hover:bg-[#0A4F3E] dark:bg-[#10B981] dark:hover:bg-[#059669] text-white text-[15px] font-medium px-6 h-14 rounded-full whitespace-nowrap transition-all"
                  >
                    {t('registerButton')}
                  </Button>

                  {/* Collector Survey button */}
                  <Button
                    variant="outline"
                    onClick={() => setShowSurveyModal(true)}
                    className="w-full border-2 border-[#0C5F4C]/40 dark:border-[#10B981]/40 text-[#0C5F4C] dark:text-[#10B981] hover:border-[#0C5F4C] dark:hover:border-[#10B981] hover:bg-[#0C5F4C]/10 dark:hover:bg-[#10B981]/10 text-[15px] font-medium px-6 h-14 rounded-full whitespace-nowrap transition-colors"
                  >
          ...
          <div className="mt-16 pb-12 relative flex justify-center px-4 sm:px-6">
            <div className="relative w-full max-w-5xl rounded-2xl overflow-hidden shadow-2xl border border-gray-200 dark:border-gray-700">
              <img
